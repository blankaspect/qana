...
////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////
                       ...
                       Prng01      prng )
        {
            ...
            // Validate arguments
            if ( (numVertices < 3) ||
                 (initialLengthRange.lowerLimit < 0.0) ||
                 (initialLengthRange.lowerLimit > initialLengthRange.upperLimit) ||
                 (normalOffsetRange.lowerLimit < 0.0) || (normalOffsetRange.lowerLimit > 1.0) ||
                 (normalOffsetRange.upperLimit < 0.0) || (normalOffsetRange.upperLimit > 1.0) ||
                 (normalOffsetRange.lowerLimit > normalOffsetRange.upperLimit) ||
                 (normalRatioRange.lowerLimit < 0.0) ||
                 (normalRatioRange.lowerLimit > normalRatioRange.upperLimit) ||
                 ((controlCoordRange != null) &&
                  ((controlCoordRange.lowerLimit < 0.0) ||
                   (controlCoordRange.lowerLimit > controlCoordRange.upperLimit))) )
            ...
            while ( vertices.size( ) < numVertices )
            {
                ...
                d = prng.nextDouble( normalRatioRange ) *
                                                StrictMath.sqrt( maxLengthSq * ((p < 0.5) ? p * (2.0 - p)
                                                                                          : 1.0 - p * p) );
                if ( prng.nextBoolean( ) )
                    d = -0.5 * d;
...
////////////////////////////////////////////////////////////////////////
                         ...
                         Kind imageKind )
    {
        ...
        DoubleRange initialLengthRange =
                                    new DoubleRange( INITIAL_LENGTH_RANGE.lowerLimit * (double)cellSize,
                                                     INITIAL_LENGTH_RANGE.upperLimit * (double)cellSize );
        DoubleRange controlCoordRange =
                                    new DoubleRange( CONTROL_COORD_RANGE.lowerLimit * (double)cellSize,
                                                     CONTROL_COORD_RANGE.upperLimit * (double)cellSize );
        ...
        for ( int i = 0; i < numColumns * numRows; ++i )
        {
            ...
            colour = new Color( colour.getRed( ),
                                StrictMath.round( (float)colour.getGreen( ) * GREEN_FACTOR ),
