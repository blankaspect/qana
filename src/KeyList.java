...
import uk.org.blankaspect.crypto.FileEncrypter;
...
import uk.org.blankaspect.util.ByteDataList;
...
////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////
            ...
            throws AppException
        {
            ...
            int nameOffset = NumberUtilities.bytesToUIntLE( data, offset, length );
            offset += length;
            ...
            kdfParamsVer = NumberUtilities.bytesToUIntLE( data, offset, length );
            offset += length;

            // Parse field: KDF parameters, CEK generation
            length = KDF_PARAMS_GEN_FIELD_SIZE;
            kdfParamsGen = NumberUtilities.bytesToUIntLE( data, offset, length );
            offset += length;

            // Parse field: allowed ciphers
            length = ALLOWED_CIPHERS_FIELD_SIZE;
            allowedCiphers = NumberUtilities.bytesToUIntLE( data, offset, length );
            offset += length;

            // Parse field: preferred cipher
            length = PREFERRED_CIPHER_FIELD_SIZE;
            preferredCipher = NumberUtilities.bytesToIntLE( data, offset, length );
            offset += length;
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        {
            return ( (name == null) ? (key.name == null) ? index - key.index : -1
                                    : (key.name == null) ? 1 : name.compareTo( key.name ) );
        }
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        private byte[] toByteArray( StringTable stringTable )
        {
            ...
            int length = NAME_OFFSET_FIELD_SIZE;
            NumberUtilities.intToBytesLE( stringTable.add( name ), buffer, offset, length );
            offset += length;

            // Set field: KDF parameters, verification
            length = KDF_PARAMS_VER_FIELD_SIZE;
            NumberUtilities.intToBytesLE( kdfParamsVer, buffer, offset, length );
            offset += length;

            // Set field: KDF parameters, CEK generation
            length = KDF_PARAMS_GEN_FIELD_SIZE;
            NumberUtilities.intToBytesLE( kdfParamsGen, buffer, offset, length );
            offset += length;

            // Set field: allowed ciphers
            length = ALLOWED_CIPHERS_FIELD_SIZE;
            NumberUtilities.intToBytesLE( allowedCiphers, buffer, offset, length );
            offset += length;

            // Set field: preferred cipher
            length = PREFERRED_CIPHER_FIELD_SIZE;
            NumberUtilities.intToBytesLE( preferredCipher, buffer, offset, length );
            offset += length;
...
////////////////////////////////////////////////////////////////////////
    ...
    {
        byte[] key = Fortuna.keyStringToBytes( passphrase );
        byte[] salt = App.getInstance( ).getRandomBytes( SALT_SIZE );
...
////////////////////////////////////////////////////////////////////////
        ...
        throws AppException
    {
        ...
        if ( NumberUtilities.bytesToUIntLE( data, offset, length ) != FILE_ID )
            throw new AppException( ErrorId.NOT_A_KEY_FILE );
        offset += length;

        // Parse field: version number
        length = VERSION_FIELD_SIZE;
        int version = NumberUtilities.bytesToUIntLE( data, offset, length );
        offset += length;
        ...
        int numKeys = NumberUtilities.bytesToUIntLE( data, offset, length );
        offset += length;
    ...
    private byte[] createHeader( )
    {
        ...
        int length = ID_FIELD_SIZE;
        NumberUtilities.intToBytesLE( FILE_ID, buffer, offset, length );
        offset += length;

        // Set field: version number
        length = VERSION_FIELD_SIZE;
        NumberUtilities.intToBytesLE( VERSION, buffer, offset, length );
        offset += length;

        // Set field: string table offset
        length = STRING_TABLE_OFFSET_FIELD_SIZE;
        NumberUtilities.intToBytesLE( HEADER_SIZE + keys.size( ) * Key.SIZE, buffer, offset, length );
        offset += length;

        // Set field: number of keys
        length = NUM_KEYS_FIELD_SIZE;
        NumberUtilities.intToBytesLE( keys.size( ), buffer, offset, length );
        offset += length;
