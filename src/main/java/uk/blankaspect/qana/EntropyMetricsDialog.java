...
////////////////////////////////////////////////////////////////////////
	...
	public static void showDialog(Component          parent,
								  ...
								  EntropyAccumulator entropyAccumulator)
	{
		new EntropyMetricsDialog(GuiUtils.getWindow(parent), prng, entropyAccumulator);
	}
...
////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////
		...
		protected void paintComponent(Graphics gr)
		{
			...
			gr2d.setColor(INDEX_BACKGROUND_COLOUR);
			gr2d.fillRect(rect.x, y, rect.width, getHeight() - y);
			...
			if (frequencyHeights != null)
			{
				...
				for (int i = 0; i < frequencyHeights.length; i++)
				{
					...
					gr2d.setColor((barHeight == 0) ? BAR_COLOUR0 : (barHeight < 0) ? BAR_COLOUR1 : BAR_COLOUR2);
					gr2d.fillRect(x, y1, barWidth, y2 - y1 + 1);
					x += barInterval;
				}
			}
			...
			{
				// Draw index marker
				gr2d.setColor(MARKER_COLOUR1);
				gr2d.drawLine(x, y, x, y + INDEX_MARKER_HEIGHT - 1);
				...
				if (i < bitIndices.size())
				{
					...
					gr2d.setColor(INDEX_COLOUR);
					gr2d.drawString(str, x - (strWidth - 1) / 2, y + INDEX_MARKER_HEIGHT + fontMetrics.getAscent());
				}
	...
	////////////////////////////////////////////////////////////////////
				...
				if ((y >= 0) && (y < getTotalPlotHeight()) && (y % PLOT_INTERVAL < PLOT_HEIGHT))
				{
					...
					if ((x >= 0) && (x < NUM_FREQUENCIES))
					{
						...
						if (frequencyStrs[bitIndex][seqIndex] != null)
						{
							...
							buffer.append(NumberUtils.uIntToBinString(seqIndex, 8, '0'));
							buffer.append(" : ");
		...
		protected void paintComponent(Graphics gr)
		{
			...
			{
				// Draw label
				int y = plotY + FontUtils.getBaselineOffset(PLOT_HEIGHT - 1, gr2d.getFontMetrics());
				gr2d.setColor(TEXT_COLOUR);
				gr2d.drawString(BIT_STR + bitIndices.get(i), LEFT_MARGIN, y);
		...
		protected void paintComponent(Graphics gr)
		{
			...
			if (lengthHeights != null)
			{
				...
				for (int i = 0; i < lengthHeights.length; i++)
				{
					...
					gr2d.setColor(canReseed ? BAR_COLOUR1 : BAR_COLOUR2);
					gr2d.fillRect(x, y1, BAR_WIDTH, y2 - y1 + 1);
					x += BAR_INTERVAL;
				}
			}
			...
			for (int i = 0; i < NUM_POOLS; i++)
			{
				if (i % INDEX_INTERVAL == 0)
				{
					// Draw index marker
					gr2d.setColor(MARKER_COLOUR1);
					gr2d.drawLine(x, y, x, y + INDEX_MARKER_HEIGHT + INDEX_MARKER_EXTRA_HEIGHT - 1);

					// Draw text
					String str = Integer.toString(i);
					gr2d.setColor(INDEX_COLOUR);
					gr2d.drawString(str, x + 2, y + INDEX_MARKER_HEIGHT + fontMetrics.getAscent());
				}
				else
				{
					// Draw index marker
					gr2d.setColor(MARKER_COLOUR2);
					gr2d.drawLine(x, y, x, y + INDEX_MARKER_HEIGHT - 1);
				}
